...
import { createMeal, getChefMeals, mealExists } from "../src/services/chefService";
...
describe('Tests', () => {
    ...
    describe('Rate Meals', () => {
        ...
        it('Tries to add a new rating but had already added one', () => {
            ...
            expect(() => {rateMeal(username, mealToRate, rating)}).toThrow('User can rate the meal only once.');
        });
        ...
        it('Tries to add a new rating with a value greater than 5', () => {
            ...
            expect(() => {rateMeal(username, mealToRate, rating)}).toThrow('The rating value must be between 1 and 5.');
        });
        ...
        it('Adds a new rating and recalculates the average', () =>{
            ...
            const ratedMeal = data.getMeals().find(m => m.chefName === mealToRate.chefName && m.meal === mealToRate.meal);
    ...
    describe('Chef Service', () => {
        ...
        it('Tries to create an existing meal', () => {
            ...
            expect(() => {createMeal(chefName, meal)}).toThrow('The supplied meal already exists');
        })
